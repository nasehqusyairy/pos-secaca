...
import { GetProductStockOpnameQueryRequest, ProductOpnameResponse } from "@/app/api/product/product-opname/type";
...
const ProductStockOpname: FC<ProductOpnameProps> = () => {
    ...
    const { data, isFetching, refetch } = useGetProductOpnameQuery(param)
    const deleteEmployee = useDeleteProductAdjustmentStockMutation();

    function handleEdit(id: number) {
        router.push(`/${locale}/backoffice/stock/stock-opname/${id}/edit`)
    }

    function handleShow(id: number) {
        router.push(`/${locale}/backoffice/stock/stock-opname/${id}?show=true`)
    }
    ...
    function handleNewData() {
        router.push(`/${locale}/backoffice/stock/stock-opname/create`)
    }

    function handleDelete(row: ProductOpnameResponse) {
        showConfirmationDeletePermanentAlert(`produk opname dengan id ${row?.id}`, () => {
            const id = row.id ?? 0;

            deleteEmployee.mutate(id, {
                onSuccess: () => {
                    showToastSuccess("Berhasil Hapus stok opname");
                    refetch();
                },
                onError: () => {
                    showToastError("Gagal Hapus stok opname");
                }
            });
        })
    }
    ...
    const columns = ProductOpnameColumns({ 
        onEdit: handleEdit,
        onShow: handleShow,
    ...
    return (
        <>
            <div>
                <PageTitleSetting title={title} subtitle="Atur stok opname"></PageTitleSetting>
            </div>
            ...
            <div className="container mx-auto">
                {
                    isFetching ? <SkeletonSimple /> : <ProductOpnameTable 
                        columnShow={ProductOpnameColumnShow}
                        columns={columns}
                        data={data?.data.data ?? []}
                    />
                }
...
export default ProductStockOpname;