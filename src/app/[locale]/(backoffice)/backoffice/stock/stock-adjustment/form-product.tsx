...
import { getProductStockOne, getProductStocks } from "@/app/api/product/api";
...
const ProductAdjustmentStockColumns = [
    "No Urut", "Barcode", "Nama Produk", "Kategori",
    "Harga Jual", "Stok", "Terhitung", "Perbedaan",
    "Total"
];

interface FieldProductAdjustmentStockProps {
...
const FieldProductAdjustmentStock: FC<FieldProductAdjustmentStockProps> = ({
    data,
    form,
    disabled,
    location_id
    ...
    useEffect(() => {
        ...
        if (!data || !data.product_adjustment_stock_details) return

        const values: any[] = []
        const products: any[] = []
        data.product_adjustment_stock_details.forEach((detail) => {
            values.push({
                id: detail.id,
    ...
    return (
        <div>
            ...
            <Table className="border rounded-xl mt-4">
                <TableHeader>
                    <TableRow>
                        {ProductAdjustmentStockColumns.map(
                            ...
                            <TableRow key={i}>
                                ...
                                { 
                                    !disabled && <TableCell className={cn("w-[100px]")}>
                                        <Button size="icon" variant="outline" onClick={() => handleDelete(i, product?.id || 0)}>
...
export default FieldProductAdjustmentStock;