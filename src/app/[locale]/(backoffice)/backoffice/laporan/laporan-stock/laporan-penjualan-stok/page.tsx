...
import { ReportStockMovementColumns } from "./column";
...
import { ReportStockMovementTable } from "./table";
import { useGetReportStockMovementQuery } from "@/app/api/laporan/report-stock-movement/queries";
import ProductDropdownSelect from "@/components/templates/Dropdowns/product-dropdown";

interface LaporanStockMovementPageProps {}

const LaporanStockMovementPage: FC<LaporanStockMovementPageProps> = () => {
    ...
    const [select_all_product, setSelectAllProduct] = useState<boolean>(true)
    const [prods, setProds] = useState<number[]>([]);
    const [exclude_prods, setExcludeProds] = useState<number[]>([]);

    const [limit, setLimit] = useState<number>(10);
    const [page, setPage] = useState<number>(1);

    const { data, isFetching, refetch } = useGetReportStockMovementQuery({ 
        select_all_product, prods, exclude_prods,
        start_at, end_at, locs, exclude_locs, select_all_location, limit, page, keyword
    });
    ...
    const columns = ReportStockMovementColumns({})
    return (
        <>
            <div>
                <PageTitleSetting title={t("laporanPergerakanStok")} />
            </div>
            ...
            <div className="container mx-auto">
                <div className="w-full flex items-center rounded-md py-1 my-5">
                    ...
                    <div className="flex flex-col gap-2 ml-4">
                        <ProductDropdownSelect
                            multiSelect
                            defaultSelectAll
                            handleSelectAllChange={setSelectAllProduct}
                            handleIdsChange={setProds}
                            handleExcludeIdsChange={setExcludeProds}
                            key="prod-1"
                        />
                    </div>
                    <div className="flex flex-col gap-2 ml-4">
                ...
                {
                    isFetching ? <SkeletonSimple /> : <ReportStockMovementTable columns={columns} data={data?.data.data} />
                }
...
export default LaporanStockMovementPage;