...
import { ReportStockCardColumns } from "./column";
...
import { ReportStockCardTable } from "./table";
import { useGetReportStockCardQuery } from "@/app/api/laporan/report-stock-card/queries";
...
const LaporanStockCardPage: FC<LaporanStockCardPageProps> = () => {
    ...
    const [select_all_location, setSelectAllLocation] = useState<boolean>(false)
    const [loc, setLoc] = useState<number>(0);
    const [keyword, setKeyword] = useState<string>('');
    const [select_all_product, setSelectAllProduct] = useState<boolean>(false)
    const [prod, setProd] = useState<number>(0);

    const [limit, setLimit] = useState<number>(10);
    const [page, setPage] = useState<number>(1);

    const { data, isFetching, refetch } = useGetReportStockCardQuery({ 
        select_all_product, prods: [prod],
        select_all_location, locs: [loc],
    ...
    const columns = ReportStockCardColumns()
    return (
        <>
            <div>
                <PageTitleSetting title={t("laporanKartuStok")} />
            </div>
            ...
            <div className="container mx-auto">
                ...
                {
                    isFetching ? <SkeletonSimple /> : <ReportStockCardTable columns={columns} data={data?.data.data} />
                }
