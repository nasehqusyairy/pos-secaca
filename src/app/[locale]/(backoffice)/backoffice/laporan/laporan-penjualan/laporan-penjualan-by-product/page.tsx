...
import { ReportSalesByProductColumns } from "./column";
...
import { useGetReportSalesByProductQuery } from "@/app/api/laporan/report-sales-by-product/queries";
import { ReportSalesByProductTable } from "./table";
...
const LaporanPenjualanPage: FC<LaporanPenjualanPageProps> = () => {
    ...
    const { data, isFetching, refetch } = useGetReportSalesByProductQuery({ start_at, end_at, locs, exclude_locs, select_all_location, limit, page, keyword, discounted });
    ...
    useEffect(() => {
        ...
        if (dateRange.to) {
            ...
            setEndAt(endOfDay(dateRange.from ?? dateFrom))
        }
    }, [dateRange]);
    ...
    const columns = ReportSalesByProductColumns()
    return (
        <>
            <div>
                <PageTitleSetting title={t("report_sales_by_product")} />
            </div>
            ...
            <div className="container mx-auto">
                ...
                {
                    isFetching ? <SkeletonSimple /> : <ReportSalesByProductTable columns={columns} data={data?.data.data} />
                }
