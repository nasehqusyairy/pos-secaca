...
const InvoicePage: FC<InvoicePageProps> = () => {
  ...
  const {data: dataInvoicesWithCursor, isPending: isPendingInvoices, refetch: refetchInvoices} = useGetInvoicesQueryWithCursor(getInvoicesQueryParams)
  const {data: dataInvoiceDetails, isPending: isPendingInvoice, refetch: refetchInvoiceDetail} = useGetInvoiceDetailQuery(idInvoice)
  ...
  const handleNextPage = (cursor?: string) => {
    setCursor(cursor ?? nextCursor)
  }

  const handlePrevPage = (cursor?: string) => {
  ...
  const totalHarga = (dataInvoiceDetail: any) => {
      ...
      <section className={`border rounded-lg md:w-1/2 xl:w-1/3 md:overflow-y-auto md:sticky md:top-10 md:min-h-[calc(100vh-120px)] md:overflow-x-hidden md:block ${menuMobile === 'daftar' ? 'block' : 'hidden'}`}>
        <div className="font-semibold text-base md:text-lg bg-primary p-4 text-white flex justify-between items-center">
          ...
          <Badge className="bg-white text-primary">
            ...
            ? <p className="text-xs md:text-base">Memuat data...</p> 
            : (
            ...
            <div className="p-4 ">
              ...
              <div className="w-2/3 mt-2 text-slate-600">
                <table className="w-full text-xs md:text-sm">
                  <tbody>
                    ...
                    {/* <tr>
                      <td>Tipe Pesanan</td>
                      <td>: {dataInvoiceDetail?.order_type_name}</td>
                    </tr> */}
                    {/* <tr>
                      <td>Harga Gross</td>
                      <td>: {formatRupiah(dataInvoiceDetail?.gross_sales as number)}</td>
                    </tr>
                    <tr>
                      <td >Harga Net</td>
                      <td className="font-semibold text-primary">: {formatRupiah(totalHarga(dataInvoiceDetail))}</td>
                    </tr> */}
                    <tr>
                      <td>Catatan</td>
              ...
              <div className="mt-10">
                <p className="text-sm md:text-base font-semibold">Daftar produk ({dataInvoiceDetail?.sale_transaction_details.length ?? 0})</p>
                {
                ...
                <div className="flex justify-between mt-2">
                  ...
                  <p className="text-xs md:text-sm font-bold">
                    {dataInvoiceDetail?.sale_transaction_details.reduce((n: number, item: any) => n + item?.quantity, 0) }
                  </p>
                </div>
                {
                ...
                {
                  additionalFee((dataInvoiceDetail?.discount_amount + dataInvoiceDetail?.promo_amount) * -1, t('adjustment_discount'))
                }
                {
...

              {
                dataInvoiceDetail?.sale_refunds && (
                  <div className="mt-10">
                    {dataInvoiceDetail?.sale_refunds?.length > 0 && <p className="text-sm md:text-base font-semibold mb-2">Daftar Pengembalian</p>}
                    {
                      dataInvoiceDetail?.sale_refunds.map((item: any, i: number) => {
                        return (
                          <div key={i}>
                            <p className="text-xs md:text-sm">Tanggal Pengembalian: {item?.refund_at && formatDateToStringDate(new Date(item?.refund_at), true)}</p>
                            {
                              item.sale_refund_details &&  item.sale_refund_details.map((item: any, i:number) => (
                                <div key={i} className="flex justify-between mt-2">
                                  <p className="text-xs md:text-sm">{item?.product_name}</p>
                                  <p className="text-xs md:text-sm">{item?.quantity} x {formatRupiah(item?.sell_price)}</p>
                                </div>
                              ))
                            }
              ...
              {
                idInvoice !== 0 && (
                  <div className="bottom-2 right-2 w-full ">
                    <Separator className="my-2" />
                    <div className="flex justify-end gap-2">
                      <Button type="button" className="bg-yellow-400 hover:bg-yellow-500">
                        <FaPrint className="mr-2"/>
                        <p className="font-semibold text-xs md:text-base" onClick={() => onPrint(dataInvoiceDetail.id)}>Cetak</p>
                      </Button>
                      {
                        dataInvoiceDetail && dataInvoiceDetail.sale_refunds.length == 0 && (
      ...
      {/* Section Menu for mobile UI */}
      {
        <div className="md:hidden fixed bottom-8 w-full bg-white p-2">
          ...
          <div className="flex justify-between items-center">
            ...
            >
              <p className={`text-xs md:text-base ${menuMobile === 'invoice' && 'text-primary'} flex items-center gap-1`}><AiFillProduct /> List Struk</p>
            </Button>
            <Button
              type="button"
              className="w-1/2"
            ...
            >
              <p className={`text-xs md:text-base ${menuMobile === 'daftar' && 'text-primary'} flex items-center gap-1`}><TbFileInvoice /> Detail Struk</p>
            </Button>
          </div>
        </div>
      }
