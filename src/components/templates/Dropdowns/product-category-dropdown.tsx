...
import { useGetDropdownProductCategoryQuery } from "@/app/api/dropdown/query";
import { capitalizeWords } from "@/lib/helpers";
import { useTranslations } from "next-intl";
import { ProductCategoryDropdown } from "@/types/dropdown";
...
interface ProductDropdownProps {
    ...
    defaultValue?: ProductCategoryDropdown | null,
    ...
    handleValueChange?: (value: ProductCategoryDropdown) => void,
}

const ProductCategoryDropdownSelect: FC<ProductDropdownProps> = (props: ProductDropdownProps) => {
    const t = useTranslations();

    const [selectAll, setSelectAll] = useState(props.defaultSelectAll ?? false);
    const [keyword, setKeyword] = useState('');
    const [value, setValue] = useState<ProductCategoryDropdown | null>(props.defaultValue ?? null);
    ...
    const { data, isFetching, isFetchingNextPage, hasNextPage, fetchNextPage, refetch } = useGetDropdownProductCategoryQuery(10, keyword, props.defaultSelectedIds ?? []);
    ...
    const handleSelect = (value: ProductCategoryDropdown) => {
        setValue(value)
    ...
    const labelButton = () => {
        if (!props.multiSelect && value) {
            ...
            return t('select_exclude_count_your', { count: excludeIds.length, name: 'Kategori' })
        } else if (!selectAll && ids.length > 0) {
            return t('select_count_your', { count: ids.length, name: 'Kategori' })
        } else if (selectAll) {
            return t('all_your', { name: 'Kategori' })
        }

        return t('select_your', { name: 'Kategori' })
    }

    const itemList = (group: any) => {
        if (!group.data) return (<></>)

        return group.data.data.map((value: ProductCategoryDropdown) => (
            <CommandItem
                key={value.id}
        ...
        <Popover>
                ...
                <PopoverContent className="p-0" onScroll={handleScroll} onScrollCapture={handleScrollCapture}>
                    <Command shouldFilter={false}>
                        <CommandInput
                            placeholder={t('search_name', { name: 'Kategori' })}
                            onValueChange={setKeyword}
                            inputMode="text"
                        />
                        <CommandList>
                            <CommandEmpty>
                                {(isFetching || isFetchingNextPage) ? t('loading') : t('not_found_name', {name: 'Kategori'})}
                            </CommandEmpty>
                            {!props.multiSelect || (isFetching || isFetchingNextPage) ? <></> : <CommandItem onSelect={handleSelectAll}>
                                {checkMark(selectAll)} { t('all_your', { name: 'Kategori'}) }
                            </CommandItem>}
...
export default ProductCategoryDropdownSelect;
